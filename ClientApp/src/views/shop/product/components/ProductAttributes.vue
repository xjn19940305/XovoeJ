...
function updateValue(key: keyof Props, value: boolean) {
  if (key === 'isHot') emit('update:isHot', value)
  else if (key === 'isNew') emit('update:isNew', value)
  else if (key === 'isRecommend') emit('update:isRecommend', value)
  else if (key === 'isEnabled') emit('update:isEnabled', value)
}
</script>

<template>
  <div class="product-attributes">
    <el-checkbox :model-value="isHot" @change="updateValue('isHot', $event as boolean)" border>
      <span class="toggle-item">
        <FaIcon name="i-heroicons-solid:fire" class="size-4 text-orange-500" />
        热门
      </span>
    </el-checkbox>
    <el-checkbox :model-value="isNew" @change="updateValue('isNew', $event as boolean)" border>
      <span class="toggle-item">
        <FaIcon name="i-heroicons-solid:sparkles" class="size-4 text-green-500" />
        新品
      </span>
    </el-checkbox>
    <el-checkbox :model-value="isRecommend" @change="updateValue('isRecommend', $event as boolean)" border>
      <span class="toggle-item">
        <FaIcon name="i-heroicons-solid:thumb-up" class="size-4 text-blue-500" />
        推荐
      </span>
    </el-checkbox>
    <el-checkbox v-if="isEnabled !== undefined" :model-value="isEnabled" @change="updateValue('isEnabled', $event as boolean)" border>
      <span class="toggle-item">
